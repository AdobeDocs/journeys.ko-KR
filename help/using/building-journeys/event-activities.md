...
1. 지정된 시간 제한 내에 이벤트가 수신되지 않을 때 개인 사용자를 시간 제한 경로로 보내려면 **[!UICONTROL Set the timeout path]** 옵션을 선택합니다. 이 옵션이 활성화되지 않으면 제한 시간에 도달하면 개인에 대한 여정이 종료됩니다.
...
시간 제한은 다음에 위치하는 모든 이벤트에 적용됩니다. **[!UICONTROL Wait]** 활동. 지정된 시간 제한 전에 이벤트가 수신되지 않으면 개인은 하나의 시간 제한 경로로 전환되거나 여정을 종료합니다.
