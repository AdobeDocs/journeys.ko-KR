...
1. **/list/properties** API를 호출하여 최신 여정 버전의 UID 여정 및 UID을 검색합니다. 필요한 경우 **/여정/`{uid}`/최신**&#x200B;에 전화하여 최신 여정 버전의 UID을 찾을 수 있습니다.
...
1. **Adobe I/O에서 [!DNL Journey Orchestration] 서비스**&#x200B;에 대한 새 통합을 만들고 구성합니다. Journey Orchestration 및 Adobe Experience Platform의 경우 제품 프로필 액세스가 필요합니다. 그러면 자격 증명이 생성됩니다(API 키, 클라이언트 암호 등).
...
이 API를 사용하면 UID 및 모든 관련 개체(여정, 이벤트, 데이터 소스, 필드 그룹, 사용자 지정 작업)로 식별되는 여정 버전을 해당 uid로 내보낼 수 있습니다.
...
* 다음 객체는 내보내지지만 대상 환경에서는 가져오지 않습니다. Journey Orchestration에서 자동으로 관리하는 시스템 리소스입니다. &quot;INSERT_SECRET_HERE&quot;를 바꿀 필요가 없습니다.
...
* 가져오는 동안 대상 환경(인스턴스 또는 샌드박스)에서 고유성을 보장하기 위해 새 UID 및 새 이름으로 여정 개체가 만들어집니다.
